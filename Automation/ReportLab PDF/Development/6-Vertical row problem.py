...
json_path = base_dir + "\\json\\input1.json"
with open(json_path) as file:
    data = json.load(file)

order_details = data.get('OrderDetails')
number_invoice = order_details.get('Order_Number__c')
date_invoice = order_details.get('LastModifiedDate')
date_invoice = date_invoice.split('T')[0]

date_order = order_details.get('ActivatedDate')
date_order = date_order.split('T')[0]
name_order = order_details.get('Name')
total_amount = '$' + str(order_details['TotalAmount'])

l1_bill = order_details.get('Account').get('Name')
l2_bill = order_details.get('BillingStreet')
l3_bill = order_details.get('BillingCity')+ ', '+ order_details.get('BillingState')+ ' '
l3_bill += order_details.get('BillingPostalCode')

#adding custom fonts to RL search path
fonts_path = base_dir + "\\fonts"
TTFSearchPath.append(fonts_path)

pdf_path = base_dir + "\\pdf\\final-test-3.pdf"
